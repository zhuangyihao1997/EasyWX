...
LogicSystem::LogicSystem() {
    ...
    RegPost("/user_register", [](std::shared_ptr<HttpConnection> connection) {
        ...
        bool b_get_varify = RedisMgr::GetInstance()->Get(email_code, varify_code);
        if (!b_get_varify) {
        ...
        std::cout << "mysql register return uid = " << uid << std::endl;
        if (uid == 0 || uid == -1) {
    ...
    RegPost("/user_login", [](std::shared_ptr<HttpConnection> connection) {
        ...
        std::cout << "Gate Status gRPC 1" << std::endl;
        auto reply = StatusGrpcClient::GetInstance()->GetChatServer(userInfo.uid);

        std::cout << "Gate Status gRPC 2" << std::endl;
        if (reply.error()) {
